...
class PID_Controller(object):
  ...
  pid_global_gains =   State( 0.25, 0.25,  0.5, 200.0, 0.01, 0.01, 0.1, 10.0)
  ...
  def init(self, drone, ai, aa, cs):
    ...
    self.index = ai
    # Setup publisher
    self.pub_twist = rospy.Publisher('/uav' + str(self.index) + '/cmd_vel', Twist, queue_size=1)
    ...
    self.path_valid_duration = rospy.Duration(10.0)
    ...
    # status msg timer
    self.status_msg_timer = rospy.Timer(rospy.Duration(1.0), self.status_msg_timer_callback)

  def status_msg_timer_callback(self, event):
    if not self.good_carrot:
        rospy.logwarn("PID Controller:: bad carrot")
    if not self.path_initialized:
        rospy.logwarn("PID Controller:: path is not initialized")
    if not self.initialized:
        rospy.logwarn("PID Controller:: I am not initialized")
  ...
  def path_timer_callback(self, event):
    #rospy.logwarn("PID Controller:: path callback: requesting path")
    self.request_path()

  def request_path(self):
    if self.initialized:
        #rospy.logwarn("PID Controller:: request_path: in")
  ...
  def goal_callback(self, msg):
    #rospy.logwarn("PID_Controller::goal_callback: goal_in: %0.2f, %0.2f from loc %.2f, %.2f", msg.x,msg.y, self.cLoc.x, self.cLoc.y)
    if abs(msg.x - self.goal.x) + abs(msg.y - self.goal.y) > 1.0:
        #rospy.logwarn("PID_Controller::goal_callback: abs(%0.2f - %0.2f) + abs(%0.2f - %0.2f) = %0.2f)", msg.x, self.goal.x,msg.y, self.goal.y, abs(msg.y-self.goal.y) + abs(msg.x - self.goal.x))
        self.goal.x = msg.x
        self.goal.y = msg.y
        self.goal_initialized = True
        self.make_rviz_marker(self.goal, 99, 0.0,0.0,1.0,1.5,10.0)
        #rospy.logwarn("PID Controller:: goal_callback: requesting path to %0.2f, %0.2f", self.goal.x, self.goal.y)
        self.request_path()
  ...
  def odom_callback(self, msg):
    ...
    if self.initialized == False:
      ...
      rospy.loginfo("PID Controller::odom callback: initialized self")
  ...
  def update_carrot(self):
    ...
    if self.path_initialized and len(self.path) > 1:
  ...
  def control_global(self):
   ...
   # get error
    ...
    # don't spin at goal
    if abs(err.x) + abs(err.y) < self.control_radius:
      err.t = 0.0
      err.ts = 0.0
    ...
    #print "************************** PID Gains ********************************** "
    #self.pid_global_gains.print_state()
    ...
    pid.ts = 0.0*self.pid_global_gains.ts * err.ts + self.pid_global_gains.t * err.t
    ...
    if abs(err.t) >= pi/2.0:
      ...
      pid.xs *= (pi/2.0 - abs(err.t)) / (pi/2.0)
      pid.ys *= (pi/2.0 - abs(err.t)) / (pi/2.0)
